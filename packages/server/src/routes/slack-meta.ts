import type { SlackChannelMeta, SlackUserMeta } from "@thoughtcurrent/shared";
...
export const slackMetaRoutes = new Hono();
...
slackMetaRoutes.get("/validate", async (c) => {
	const userToken = getUserToken();
	const botToken = getBotToken();

	if (!userToken && !botToken) {
		return c.json(
			{ error: "No Slack tokens configured", tokenType: null },
			400,
		);
	}
...
slackMetaRoutes.get("/channels", async (c) => {
	if (channelCache && channelCache.expiresAt > Date.now()) {
		return c.json({ channels: channelCache.data });
	}
	...
	try {
		...
		if (!token) {
			return c.json({ error: "No Slack token configured" }, 400);
		}
...
slackMetaRoutes.get("/users", async (c) => {
	if (userListCache && userListCache.expiresAt > Date.now()) {
		return c.json({ users: userListCache.data });
	}
